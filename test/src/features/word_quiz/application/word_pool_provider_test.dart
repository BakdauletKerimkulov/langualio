...
WordEntry _makeEntry(String id, String word, {String translation = 'перевод'}) =>
    WordEntry(
      id: id,
      word: word,
...
void main() {
  group('WordPoolProvider', () {
    test('merges asset and user words', () async {
      ...
      expect(pool.map((w) => w.word), containsAll(['ability', 'achieve', 'ephemeral']));
    });

    test('user word overrides asset word with same word (case-insensitive)',
        () async {
      final assetWords = [
