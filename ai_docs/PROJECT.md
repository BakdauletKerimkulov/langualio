...
### Табличные привилегии: локально строже, чем в проде

Локальная база и прод расходятся на уровне `GRANT`, и это не баг одной из сторон, а разные
эпохи дефолтов Supabase. Перед тем как считать, что привилегия есть в обоих окружениях,
сверься с этим разделом.

- **Прод** заведён на старых дефолтах: у `anon`, `authenticated` и `service_role` стоит
  `GRANT ALL` на всех 13 таблицах. Единственное исключение — `profiles`, где миграция
  `20260721175539_assessment_server.sql` отозвала табличный `UPDATE` и вернула его колоночно
  (`nickname`, `avatar_url`). Утечки нет: доступ реально режет RLS, привилегии не помогают —
  но это защита в один слой вместо двух.
- **Локально** схема `public` несёт два набора `ALTER DEFAULT PRIVILEGES`: объекты от
  `supabase_admin` получают полный DML, объекты от `postgres` — только `Dxtm` без
  SELECT/INSERT/UPDATE/DELETE. Миграции применяются от `postgres`, поэтому каждая созданная
  миграцией таблица приходит **без** клиентских привилегий, и запрос падает с
  `permission denied for table ...`, не дойдя до политики.
- Разрыв закрывает `20260905150312_grant_client_dml_and_close_public_execute.sql`: она выдаёт
  `authenticated` ровно тот набор команд, который разрешает RLS-политика каждой таблицы,
  `service_role` — полный DML, а `anon` — ничего (ни одна политика не адресована
  неаутентифицированному пользователю). Гранты заданы явно и должны оставаться явными:
  на дефолтные привилегии для новой таблицы больше не рассчитываем.
- Та же миграция закрывает `EXECUTE` для `PUBLIC` у `get_todays_words()` и
  `upsert_word_learning_progress()`. Раньше `20260514193920_word_quiz_security.sql` отзывал
  права только у `anon`, а Postgres по умолчанию раздаёт `EXECUTE` роли `PUBLIC` — аноним
  доходил до функции через неё, и останавливал его только внутренний auth guard.
- Следствие для тестов: локальный набор строже прода, поэтому политика, случайно открытая
  роли `public` без `TO authenticated`, локально всё равно упрётся в отсутствие гранта, а в
  проде отдаст данные. Пока прод не приведён к тому же явному набору, RLS-тесты этот класс
  регрессий не ловят.
