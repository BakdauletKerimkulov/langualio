...
BEGIN
  ...
  -- Check value unchanged (column-level REVOKE means UPDATE succeeds on row
  -- but the cefr_level column is silently ignored)
  DECLARE v_after int;
  BEGIN
...
BEGIN
  ...
  IF v_count > 0 THEN
    RAISE EXCEPTION 'TEST FAILED: authenticated user updated message_count (% rows)', v_count;
  END IF;
...
BEGIN
  ...
  IF v_count > 0 THEN
    RAISE EXCEPTION 'TEST FAILED: authenticated user updated generation_count (% rows)', v_count;
  END IF;
  ...
  IF v_count != 2 THEN
    RAISE EXCEPTION 'TEST FAILED: generation_count changed to % (expected 2)', v_count;
  END IF;
