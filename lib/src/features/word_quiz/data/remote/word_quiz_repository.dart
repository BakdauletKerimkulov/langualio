...
class WordQuizRepository {
  ...
  Future<List<WordEntry>> fetchTodaysWords() async {
  ...
  /// Fetches all published words from `daily_words` (used as distractor pool).
  ...
  Future<void> saveAttempt(WordQuizAttempt attempt) async {
    try {
      ...
      log(
        'Failed to save attempt, queuing locally: $e',
  ...
  Future<void> updateLearningProgress({
    ...
    try {
      ...
      log('Failed to update learning progress: $e', name: 'WordQuizRepository');
    }
  }
  ...
  List<WordEntry>? getCachedTodaysWords() {
  ...
  Future<void> _flushPendingAttempts() async {
    ...
    for (final attemptJson in pending) {
      try {
        ...
        log('Failed to flush pending attempt: $e', name: 'WordQuizRepository');
